...
On the IOS (Swift) side, caching utilizes SwiftData's built-in caching (import SwiftData), model context, @model attribute, and model container. Models are the data structure that is saved in the model context and stored in the model container. @model allows us to take our EventDTO (Json parsed event data from API) and turn it into an @model (eventModel) datastructure. This gets help from ArrayTransformer.swift, to take care of tags and organizations. Model container takes in a configuration, which notes: what kind of models are stored in the context, whether or not these will persist between app runs, and other such storage information. Model context is what is brought into other classes to be able to save to that context and thus that container. Thus, persistent states for events are possible. These events are stored in the local cache (the event context), and when updating (every 5 minutes), or force refreshing, only changes events that have been changed, and adding new events. 
